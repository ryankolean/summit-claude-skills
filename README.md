...
| **[gh-repo-create](skills/gh-repo-create/SKILL.md)** | Creates and initializes a new GitHub repo under the `ryankolean` org with production-ready defaults using the `gh` CLI. Private by default, sets up README, `.gitignore`, license, `CLAUDE.md`, and the `.claude/dispatch/` scaffold. Chains into architect-plan-for-dispatch and any code-writing skill that needs a fresh repo to target. | "create a repo", "new repo", "set up a GitHub repo", "spin up a repo for this" |
...
| **[socratic-check](skills/socratic-check/SKILL.md)** | Pre-decision 7-prompt Socratic checklist that attacks the user's own reasoning before they commit — assumptions, evidence, premortem, counter-argument, bias, framing, second-order consequences. Anti-vending-machine habit: sharpens the user's thinking rather than attacking the idea itself. Sibling to `devils-advocate` (which attacks the idea) and `interrogate` (which scopes ambiguity). | "socratic check", "run the 7 prompts", "pre-decision check", "stress my reasoning" |
...
| **[website-design-extractor](skills/website-design-extractor/SKILL.md)** | Extracts a website's design DNA (layout, colors, typography, animations) into a fixed-schema Markdown spec via Claude in Chrome, desktop + mobile, saved to a local catalog. | "extract the design from {URL}", "catalog this site", "I want a site like {URL}" |
...
| **[design-a-system](skills/design-a-system/SKILL.md)** | Designs a system from concept to implementation-ready blueprint across four classes (UI, code architecture, business process, automation pipeline). Five-phase workflow: scope, parts, interfaces and flow, failure and evolution, hand-off. Produces a design document and chains into `architect-plan-for-dispatch`, `devils-advocate`, or `design-doc-review`. Sibling to `systemize` — this skill designs systems that do not yet exist; `systemize` promotes an existing action into a component. | "design a system for...", "how should this system work", "sketch a system for X", "what should this system look like", "design this system" |
| **[chat-to-code-prompt](skills/chat-to-code-prompt/SKILL.md)** | Packages the current Claude.ai chat into a clipboard-ready, self-contained prompt for a fresh Claude Code session. Asks at activation whether it's a task, plan, or context handoff. | "chat-to-code-prompt", "send to Claude Code", "make a CC prompt", "handoff to CC" |
| **[context-summary](skills/context-summary/SKILL.md)** | Pack the active tasking of a Claude.ai chat into a token-efficient handoff brief for a fresh session. Captures only what's needed to continue the work, know the next action, and know when it's complete — no archival filler. Outputs a single fenced markdown block for tap-and-hold copy. | "context summary", "pack the context", "fresh context handoff", "wrap this for a new chat" |
| **[brain-dump](skills/brain-dump/SKILL.md)** | Capture, structure, and organize ideas from any source (URLs, screenshots, text, voice transcripts, calendar inbox events, iOS Shortcut payloads) into the ryankolean/mindspace GitHub repo. Auto-schedules High-priority items via Google Calendar reminder-hub conventions. Self-improves by appending to learnings.md after every session. | "brain dump", "capture this", "add to mindspace", "process my inbox", "mindspace review", "what did I capture about X", or sharing a URL/screenshot/idea fragment without explicit instructions |
| **[agent-architect](skills/agent-architect/SKILL.md)** | Designs, captures, and deploys Claude Code subagents (`.claude/agents/`). Three modes: Architect (scope from description), Lock (capture chat pattern), Push (deploy with container fallback). Complements the built-in `/agents` wizard — use when scoping in claude.ai, capturing a chat pattern, deploying to a tracked repo, or applying anti-pattern enforcement. | "design a subagent for...", "lock this as a subagent", "agent-architect" |
| **[watch-video](skills/watch-video/SKILL.md)** | Lets Claude actually watch a video or reel. Downloads it, extracts timestamped frames, and builds a timestamped transcript (native captions first, local whisper.cpp fallback), then reads the frames as images. Four cost modes from transcript-only to scene-detected frames. | "what happens in this reel", "summarize this video", "what does he say at 2:30" |
| **[ig-saves](skills/ig-saves/SKILL.md)** | Turns Instagram saved posts into mindspace idea cards. Parses saves from Instagram's official data export (no session-cookie scraping, no ToS violation), enriches each with its caption plus a watch-video transcript, and stages them for triage. Second phase generates hooks, outlines, and per-platform breakdowns. | "process my Instagram saves", "ingest my saves export", "turn this saved reel into content ideas" |
| **[career-launchpad](https://github.com/ryankolean/career-launchpad)** (chain) | 4-skill chain for end-to-end job search: LinkedIn rebuild, role matching + tailoring, interview prep | "set up my career launchpad" |
...
| **[cli-yt-dlp](skills/cli-yt-dlp/SKILL.md)** | Download video, audio, captions, and metadata from 1000+ sites | [yt-dlp/yt-dlp](https://github.com/yt-dlp/yt-dlp) |
...
prompt-architect → Build the Claude prompt for each automatable step
       ↓
chat-to-code-prompt → Package a chat into a ready-to-paste Claude Code prompt
       ↓
...
9. CHAT-TO-CODE-PROMPT: When I say "chat-to-code-prompt", "send to Claude Code", "make a CC prompt", or "handoff to CC", package the current Claude.ai chat into a clipboard-ready, self-contained prompt for a fresh Claude Code session. Ask upfront whether it's a task, plan, or context handoff. Output a single fenced markdown block at the end of the response, tap-and-hold copyable on mobile, with no references to "the chat" or "earlier" inside the prompt.

10. BRAIN-DUMP: When I say "brain dump", "capture this", "add to mindspace", or share a URL/screenshot/idea fragment without explicit instructions, capture the idea into the ryankolean/mindspace GitHub repo using the structured card format. Auto-schedule High-priority items via Google Calendar. Append learnings after each session.

11. DESIGN-A-SYSTEM: When I say "design a system for...", "how should this system work", or "sketch a system for X", run the five-phase design workflow (scope, parts, interfaces and flow, failure and evolution, hand-off) across the four system classes (UI, code architecture, business process, automation pipeline). Produce a design document, then offer to chain into a build plan, stress-test, or review.

12. SOCRATIC-CHECK: When I say "socratic check", "run the 7 prompts", or "pre-decision check", run the 7-prompt Socratic checklist against my stated position: (1) challenge assumptions, (2) test evidence, (3) premortem, (4) steelman counter-argument, (5) bias detection, (6) framing challenge, (7) second and third-order consequences. Produce a short scoreboard with strongest joint, weakest joint, and commit/shore-up/kill recommendation. Use before `devils-advocate` when the goal is sharpening my reasoning, not attacking the idea.

13. IG-SAVES: When I say "process my Instagram saves" or "ingest my saves export", parse my Instagram data export for saved posts, enrich each with its caption and a watch-video transcript, then help me triage them into mindspace idea cards. Never add session-cookie or private-API fetching — the official export is the sanctioned path.

14. WATCH-VIDEO: When I share a video, reel, or short and ask what is in it, download it and build both timestamped frames and a timestamped transcript, then read the frames as images and answer with timestamps. Default to transcript-only for anything over about five minutes, then spend frames only on the window that matters.

15. MENTOR SKILLS: I have 9 mentor coaching lenses available. When I say "ask [name]" or "[name] mode", apply that person's published decision-making frameworks as a coaching lens (not impersonation). Available: Gary Vee (execution/attention), Tim Ferriss (80/20/lifestyle design), Alex Hormozi (value equations/pricing), Naval Ravikant (leverage/specific knowledge), James Clear (atomic habits/systems), Peter Attia (longevity/healthspan), Sahil Bloom (mental models/5 types of wealth), Ray Dalio (principles/radical transparency), Brené Brown (vulnerability/courage/trust).

16. MENTOR COUNCIL: When I say "convene the council" or "mentor council", present my decision to 3-5 of the most relevant mentor lenses. Show each perspective with the specific framework applied, where they agree, where they disagree, and a synthesis.
